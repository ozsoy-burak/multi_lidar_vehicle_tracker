...
#engel_cmd_vel topiğine abone olunur ve oradan gelene veri ile gazebo modelinin hareketi sağlanır.
...
class MovingBoxController:
    def __init__(self):
        ...
        self.model_name = rospy.get_param("~model_name", "hatchback")


        self.start_x = rospy.get_param("~start_x", 28.24)
        self.start_y = rospy.get_param("~start_y", -14.72)
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            self.state.pose.position.y += self.cmd_vel_x * 0.1  # 0.1 saniyelik adım
